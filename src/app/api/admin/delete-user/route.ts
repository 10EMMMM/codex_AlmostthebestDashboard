...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (profileError && profileError.code !== 'PGRST116') {
      console.error('[delete-user] profile fetch', profileError);
      return NextResponse.json({ error: 'Unable to load profile.' }, { status: 400 });
    }
    if (rolesError) {
      console.error('[delete-user] roles fetch', rolesError);
      return NextResponse.json({ error: 'Unable to load roles.' }, { status: 400 });
    }
    if (cityError) {
      console.error('[delete-user] city fetch', cityError);
    ...
    await supabaseAdmin.from('archived_users').insert({
      ...
      archive_reason: reason && reason.length ? reason : null,
    });

    const deleteResponse = await supabaseAdmin.auth.admin.deleteUser(userId);
    if (deleteResponse.error) {
      console.error('[delete-user] delete error', deleteResponse.error);
      return NextResponse.json({ error: deleteResponse.error.message }, { status: 400 });
    }
