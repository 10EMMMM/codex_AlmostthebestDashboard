...
This file contains the canonical SQL for the first version of the dashboard data model. Statements are Postgres-compatible and assume Supabase defaults (UUID primary keys, `auth.users` for authentication).
...
```sql
-- Reference tables (declared early for FK usage)
...
```sql
-- RLS scaffolding (policies to be filled in per environment)
...
-- Add similar ALTER statements for every child table plus policies referencing user roles/assignments.
```

> Apply these statements via Supabase CLI migrations, then seed `cities`, `cuisines`, and initial status snapshots before wiring the React pages to Supabase queries.
