...
export async function GET(request: Request) {
  ...
  try {
    ...
    const responsePayload = ids.map((id) => ({
      id,
      label: profileLookup.get(id) ?? emailLookup.get(id) ?? "BDR",
    }));

    return NextResponse.json({ bdrs: responsePayload });
  } catch (error) {
    console.error("Unexpected error loading BDR directory:", error);
    return NextResponse.json(
